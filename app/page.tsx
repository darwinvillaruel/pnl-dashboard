...
export default function Home() {
  ...
  const fetchData = useCallback(async (r: Region, m: number) => {
    ...
    try {
      const res = await fetch(
        `/api/sheets?region=${encodeURIComponent(r)}&month=${m}`,
      );
  ...
  return (
    <div className="min-h-screen" style={{ background: "#080c14" }}>
      ...
      <Header
        onRefresh={() => fetchData(region, month)}
        onExport={exportCSV}
        loading={loading}
      />
      ...
      <main className="relative px-4 py-8 mx-auto max-w-screen-2xl sm:px-6">
        ...
        {/* Error banner */}
        {error && (
          <div className="flex items-start gap-3 px-4 py-3 mb-6 text-sm text-red-400 border rounded-xl border-red-500/30 bg-red-500/10">
            <svg
        ...
        <div
          ...
          {/* Month selector */}
          <div className="flex items-center gap-1 p-1 border rounded-xl border-slate-700/50 bg-surface-800/60">
        ...
        {/* Controls */}
        <div
          className="flex flex-wrap items-center gap-3 mb-5 animate-fade-up"
        ...
        <div
          ...
          {loading ? (
            ...
            <div className="py-16 text-center border rounded-2xl border-slate-800/40 bg-surface-800/30">
              <p className="text-slate-500">No verticals match your filters.</p>
            </div>
          ) : (
            filteredVerticals.map((v) => (
              <VerticalCard key={v.id} vertical={v} month={month} year={year} />
            ))
          )}
        </div>

        {/* Footer */}
        <div className="py-6 mt-12 text-center border-t border-slate-800/40">
